...
/**
*@author: JJyy
*@todo: get the user_id for some ad channel, return user_id for get transform
		data in WEB
*@param: redis client, channel_name
*
**/
func userIds(c *redis.Client, ad_channel string, start_time int64, end_time int64) []string {
	...
	for _, v := range res {
		...
		if start_time == 0 && end_time == 0 {
			...
			if get_start >= start_time && get_start <= end_time {
				user_id_sli = append(user_id_sli, get_start_str[0]+":"+spl_sli[4])
			}
